...
class StockImport(models.TransientModel):
	...
	def import_file_button(self):
		...
		if self.inventory_option == 'add' :
			stock_inventory = inventory_obj.create({'name':self.name,'location_ids':[self.location_id.id],'accounting_date' : self.account_date, 'start_empty': True})
			...
			if not stock_inventory :
				...
				raise ValidationError(_('"%s" Inventory is not available.')%(self.name))
				...
				if no <= 0:
					...
					if data :
						...
						if self.import_prod_option == 'name' :
							...
							if product_list and product_rec and stock_inventory:
								stock_inventory.write({'product_ids':[(6,0,product_list)]})
				...
				if no!= 0:
						...
						if values :
							...
							if self.import_prod_option == 'name' :
								...
								if product_rec:
									product_list.append(product_rec.id)
								if product_list and stock_inventory and product_rec:
									stock_inventory.write({'product_ids':[(6,0,product_list)]})
							...
							if self.import_prod_option == 'barcode':
								...
								if product_rec:
									product_list.append(product_rec.id)
								if product_rec and product_list and stock_inventory:
									stock_inventory.write({'product_ids':[(6,0,product_list)]})
